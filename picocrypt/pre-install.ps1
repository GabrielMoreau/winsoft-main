...
ForEach ($Key in Get-ChildItem -Recurse $UninstallKeys) {
	...
	} ElseIf ($UninstallString -match 'uninstall\.bat') {
		$Exe = $UninstallString
		$Args = '/NotUse'
		If (!(Test-Path -Path "$Exe")) {
			Write-Output "Error: batch not exists $Exe"
			Continue
		}
	} Else {
		Continue
	...
	Run-Exec -FilePath "$Exe" -ArgumentList "$Args" -Name "$RefVersion"
}
