...
#copie des fichiers ij.jar  ImageJ.exe  jre  luts  macros  plugins
...
Copy-Item -LiteralPath 'ImageJ.exe'     -Destination "${Env:ProgramData}\ImageJ" -Force
...
If ((Test-Path -LiteralPath $StartMenu) -And (Test-Path -LiteralPath "${Env:ProgramData}\ImageJ\ImageJ.exe")) {
	...
	$Shortcut.TargetPath = "${Env:ProgramData}\ImageJ\ImageJ.exe"
	$ShortCut.Arguments = ""
