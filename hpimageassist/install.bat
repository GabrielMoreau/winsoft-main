...
IF EXIST "%ProgramFiles%\HP\HPIA\HPImageAssistant.exe" (
  ...
  IF EXIST "%ALLUSERSPROFILE%\Microsoft\Windows\Start Menu\Programs" (
    %pwrsh% -Command "$WS = New-Object -ComObject WScript.Shell; $SC = $WS.CreateShortcut('%shortcut%'); $SC.TargetPath = '%ProgramFiles%\HP\HPIA\HPImageAssistant.exe'; $SC.Save();" -NonInteractive -NoProfile
  )
