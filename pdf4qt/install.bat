...
SET "mainexe=%ProgramFiles(x86)%\PDF4QT\Pdf4QtDiff.exe"
SET "mainexe=%mainexe%;%ProgramFiles(x86)%\PDF4QT\Pdf4QtEditor.exe"
SET "mainexe=%mainexe%;%ProgramFiles(x86)%\PDF4QT\Pdf4QtPageMaster.exe"
SET "mainexe=%mainexe%;%ProgramFiles(x86)%\PDF4QT\Pdf4QtViewer.exe"
...
ScriptRunner.exe -appvscript MsiExec.exe /i "JakubMelka.PDF4QT_%softversion%.msi" /qn /norestart /L*v "%logdir%\%softname%-MSI.log" -appvscriptrunnerparameters -wait -timeout=300
IF "%RETURNCODE%"=="0" SET "RETURNCODE=%ERRORLEVEL%"
