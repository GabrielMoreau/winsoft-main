...
@ECHO [INFO] Copy avidemux.qs (need for silent uninstall)
...
IF EXIST "%ALLUSERSPROFILE%\Microsoft\Windows\Start Menu\Programs" (
  %pwrsh% -Command "$WS = New-Object -ComObject WScript.Shell; $SC = $WS.CreateShortcut('%shortcut%'); $SC.TargetPath = '%ProgramFiles%\Avidemux %softversionshort% VC++ 64bits\avidemux.exe'; $SC.Save();" -NoLogo -NonInteractive -NoProfile
  )
